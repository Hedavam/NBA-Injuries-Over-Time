...
![Kaggle Overview](images/kaggle_overview-02.png){width="100%"}
...
ggplot(months_per_year, aes(x=Year, y=Months_of_Data)) +
  ...
  geom_point() +
  theme_minimal() +
  labs(title="Plot 1: Number of Months with Recorded Injuries by Year",
       y="Number of Months",
       x="Year") +
...
```{r correlation-plots, echo=FALSE, fig.width=12, fig.height=8}
par(mfrow=c(1,2))
...
```{r rolling-stats, echo=FALSE, fig.width=8, fig.height=4}
library(zoo)
...
```{r model-pacf, echo=FALSE, fig.width=8, fig.height=4}
par(mfrow=c(1,2))
...
```{r ARIMA-xreg, echo = FALSE, fig.width=8, fig.height=4}
...
pacf(resid(auto_model_reg), main = "Plot 3.2: PACF of arima with an external regressor" )
```
...
```{r forecasting, echo=FALSE, fig.width=8, fig.height=5}
...
```{r ols-inference, echo=FALSE, fig.width=12, fig.height=8}
#display setup
par(mfrow = c(2, 2))
...
```{r ols-independence, echo=FALSE, fig.width=12, fig.height=10}
#independence assumptions
par(mfrow = c(2, 1))
pacf(resid(ols_fit), main = "5.1.2 ACF for OLS residuals")
pacf(resid(ols_fit), main = "5.1.3 PACF for OLS residuals")
par(mfrow = c(1, 1))
```

Shifting our focus to our assessment of the assumptions of regression, we see that linearity is satisified as the LOESS line on the Residuals vs. Fitted model is fairly straight suggesting a lack of patterns in the residuals. However, constant variance might be violated as the errors for smaller fitted values are pretty small compared to the error associated with larger fitted values. The slanted start to the LOESS line on the Scale-Location plot highlights this issue. Normality is also questionable as the points towards the end of our reference line are slightly curved and pretty far way from it. Lastly, independence is violated as we see an abundance on significant spikes on the ACF/PACF plots for the residuals.
...
```{r gls-inference, echo=FALSE,  fig.width=12, fig.height=10}
par(mfrow = c(2, 1))
...
```{r}
residuals_by_team <- tibble(Team = NBA_imputed$Team, Residuals = resid(gls_fit, type = "normalized"))
...
#questionable PACFs
# pacf(team_residuals, main = paste("PACF of", residuals_by_team$Team, "Residuals"))
# pacf(team_residuals, main = paste("PACF of", residuals_by_team$Team$Raptors, "Residuals"))
# pacf(team_residuals, main = paste("PACF of", residuals_by_team$Team$Hawks, "Residuals"))
# pacf(team_residuals, main = paste("PACF of", residuals_by_team$Team$Celtics, "Residuals"))
```
...
Some stand-outs are the Celtics, Heat, and Spurs, which are franchises that saw great success from 2000-2001. Based on the 95 % Confidence Intervals, for any given year, being on the Heat/Celtics is associated with approximately 1-9 more injuries. This range jumps up to 5-13 for being on the Spurs. It seems like these teams found a way to win despite their injuries. Alternatively, their players may have had lots of minor injuries or mostly their rotation players got injured as opposed to key starters.
...
```{r recovery, echo=FALSE, fig.width=8, fig.height=4}
